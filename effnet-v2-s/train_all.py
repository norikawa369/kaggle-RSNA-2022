...
def train_effnet(ds_train, ds_eval, logger, name, fold=None):
    torch.manual_seed(42)
    ...
    # load detection model parameter
    #if fold:
    #    model.load_state_dict(torch.load(os.path.join(MY_METADATA_PATH, f'segeffnetv2-f{fold}.pth'), map_location=DEVICE), strict=False)
...
if __name__ == "__main__":
    ...
    if os.path.exists(os.path.join(EFFNET_CHECKPOINTS_PATH, f'effnetv2_baseline-all-8000.pth')):
        print(f'Found cached version of effnetv2')
        #effnet_models.append(load_model(EffnetModel(), f'effnetv2_baseline-all-8000', EFFNET_CHECKPOINTS_PATH))
    else:
        with wandb.init(project='RSNA-2022', name=f'EffNet-v2_1-all-8000') as run:
            gc_collect()
            ds_train = EffnetDataSet(df_train, TRAIN_IMAGES_PATH, WEIGHTS.transforms())
            train_effnet(ds_train, None, run, f'effnetv2_1-all-8000')
